...
const CARD_BGS = ["#FCF3DB", "#E8F4FD", "#E1F3D8", "#FDE8F4", "#F0EEFF"]
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeCategory !== "Wszystkie") {
      result = result.filter((p) => {
        ...
        if (activeCategory === "Owsiane")
          return (
            lowerName.includes("owies") ||
            lowerDesc.includes("owsiany") ||
            lowerName.includes("oatly")
          )
        if (activeCategory === "Migdałowe")
          return lowerName.includes("migdał") || lowerDesc.includes("almond")
        if (activeCategory === "Kokosowe")
          return lowerName.includes("kokos") || lowerDesc.includes("kokosowy")
      ...
      <div className='max-w-7xl mx-auto px-4'>
        <div className='flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12'>
          <div className='space-y-2'>
            ...
            <h1 className='text-5xl md:text-7xl font-black text-black tracking-tight'>
              Wybierz swoje{" "}
              <span className='text-oat-yellow underline decoration-[6px] underline-offset-8'>
                paliwo
              </span>
            </h1>
          </div>
        ...
        <div className='flex flex-col lg:flex-row gap-6 mb-12 items-start lg:items-center justify-between'>
          ...
          <div className='flex items-center gap-3 w-full sm:w-auto'>
            <span className='text-sm font-bold text-neutral-500 hidden sm:block'>
              Sortuj według:
            </span>
            <select
              value={sortBy}
        ...
        <div className='mt-24 relative overflow-hidden bg-oat-green neo-border p-12 rounded-[3rem] shadow-[15px_15px_0px_#000]'>
          <div className='relative z-10 max-w-2xl'>
            ...
            <p className='text-white/90 text-lg font-bold mb-8'>
              Zapisz się do naszego newslettera i odbierz 10% rabatu na pierwsze
              zakupy!
            </p>
            <form
