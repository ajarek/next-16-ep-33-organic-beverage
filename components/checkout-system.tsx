      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2 max-w-xl">
            Podaj dane, wybierz metodę płatności i pozwól nam przygotować świeżo zaparzone napoje organiczne prosto do Twoich rąk.
          </p>
        </div>
        ...
        <div className="bg-oat-cream neo-border p-6 rounded-[1rem]">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <h3 className="font-heading text-2xl font-black">Dziękujemy, {name.split(" ")[0] || "Gościu"}!</h3>
            </div>
          </div>

          <p className="text-muted-foreground mb-4">
            Twoje zamówienie <span className="font-bold">{orderId}</span> zostało przyjęte do realizacji.
          </p>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="bg-white neo-border-sm p-4">
              <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground font-bold mb-2">Suma</p>
              <p className="font-heading text-3xl font-black text-oat-blue">{orderTotal.toFixed(2)} zł</p>
              <p className="text-sm text-muted-foreground mt-2">Płatność: {paymentOptions.find((method) => method.id === paymentMethod)?.label}</p>
            </div>
            <div className="bg-white neo-border-sm p-4">
              <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground font-bold mb-2">Dostawa</p>
              <p className="text-lg font-bold">{shipping === 0 ? "Gratis" : `${shipping.toFixed(2)} zł`}</p>
              <p className="text-sm text-muted-foreground mt-2">Wyprawka w ekologicznej torbie dołączona gratis.</p>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-lg bg-oat-blue/10 border border-black p-4 text-sm text-muted-foreground">
            ...
            <p>
              Czekamy na Ciebie z najlepszymi roślinnymi napojami. Twoje zamówienie jest już w drodze do przygotowania — spokojnie, wciąż parzymy je dla Ciebie na świeżo.
            </p>
          </div>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="w-full mt-2 min-h-[120px] rounded-lg border-2 border-black/80 bg-oat-cream px-4 py-3 text-base font-medium outline-none focus:border-oat-blue"
          ...
          <div className="rounded-2xl border-2 border-black p-4 bg-oat-cream">
            <div className="mb-3 text-sm uppercase tracking-[0.25em] font-bold text-muted-foreground">Metoda płatności</div>
          ...
          <div className="space-y-4">
            <div className="bg-white neo-border-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="font-medium text-muted-foreground">Wartość koszyka</span>
                <span className="font-bold">{totalPrice.toFixed(2)} zł</span>
              </div>
              <div className="flex items-center justify-between mb-3">
                <span className="font-medium text-muted-foreground">Dostawa</span>
                <span className="font-bold">{shipping === 0 ? "Gratis" : `${shipping.toFixed(2)} zł`}</span>
              </div>
              <div className="flex items-center justify-between pt-3 border-t-2 border-dashed border-black/20">
                ...
                <span className="font-heading font-black text-2xl text-oat-blue">{orderTotal.toFixed(2)} zł</span>
              </div>
            </div>
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border-2 border-black/50 bg-oat-cream p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs uppercase tracking-[0.25em] font-bold text-muted-foreground">Dostawa</span>
          </div>
          <p className="text-sm">
            Ekologiczna dostawa do domu lub odbiór osobisty w wybranym punkcie. Paczka spakowana z miłością do natury.
          </p>
        </div>
        ...
        <div className="rounded-2xl border-2 border-black/50 bg-oat-cream p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs uppercase tracking-[0.25em] font-bold text-muted-foreground">Prezenty</span>
          </div>
          <p className="text-sm">
            Przy zamówieniu powyżej 100 zł dorzucamy extra herbatkę niespodziankę. Złap smak natury z każdym łykiem.
          </p>
        </div>
      </div>
    </div>
